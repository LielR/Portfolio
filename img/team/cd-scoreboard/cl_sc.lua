...
function QB.Scoreboard.GetPlayerCount(self)
...
Citizen.CreateThread(function()
    ...
    WarMenu.SetMenuX("scoreboard", 0.75)
    WarMenu.SetMenuY("scoreboard", 0.03)

    WarMenu.CreateSubMenu('playeroptions', 'scoreboard', 'Player Info')

	while true do
		if WarMenu.IsMenuOpened('scoreboard') then
			if WarMenu.Button("Total:", tostring(QB.Scoreboard:GetPlayerCount()), {r = 135, g = 206, b = 250, a = 150}) then
				else
		...
		elseif WarMenu.IsMenuOpened('playeroptions') then
			...
			WarMenu.Display()
		elseif IsControlJustPressed(0, 316) then -- U by default
